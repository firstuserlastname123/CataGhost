...
func main() {
	...
	for i := 1; i < len(os.Args); i++ {
		...
		switch a {
		case "cli", "node", "server", "orchestrator", "scenario", "auth", "world-auth", "char-enum", "char-login", "world-state", "movement", "navigation", "npc-interaction", "quest-acceptance", "quest-reconnect", "quest-progress", "objective-preflight", "single-combat":
			runMode = a
			verbIdx = i
			goto foundVerb
		}
...
foundVerb:
	...
	case "single-combat":
		if err := runSingleCombat434(cliCfg, *realmName, *expectedWorld, *loginName, *instanceAddress); err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
	case "quest-progress":
		...
		if err := runQuestProgress(cliCfg, *realmName, *expectedWorld, *loginName, *instanceAddress, uint32(*inspectedQuest)); err != nil {
			fmt.Fprintf(os.Stderr, "Quest progress failed: %v\n", err)
			os.Exit(1)
		}
	...
	case "navigation":
		if err := runNavigation(cliCfg, *realmName, *expectedWorld, *loginName, *instanceAddress); err != nil {
			fmt.Fprintf(os.Stderr, "Navigation verification failed: %v\n", err)
			os.Exit(1)
		}
	case "scenario":
