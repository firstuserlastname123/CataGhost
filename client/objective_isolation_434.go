...
// Isolation is a read-only preflight, not evidence of hidden pet control state.
// The scoped QA spawns can wander 15 yards and call assistance within 10 yards.
// Use 30 yards (including margin), covering the path and both endpoints. This
// conservative clearance is not a claim that every nearby unit will attack.
...
// A caller must still prove fresh-session pet state and continuously monitor any
...
func InspectObjectiveIsolation434(observation QuestProgressObservation434, finder RouteFinder434, factions *NPCFactions434) ([]ObjectiveIsolation434, error) {
	...
	for _, candidate := range objectiveCandidates434(s, factions, objective.Target) {
		...
		for _, n := range s.Objects() {
			...
			if n.GUID == fieldGUID434(p, 0xa) && fieldGUID434(&n, 0x10) == p.GUID && fieldGUID434(&n, 0xe) == 0 {
				continue
			}
